...
export default function AdminUsers({ auth, setCurrentPage, currentPage }) {
  ...
  useEffect(() => {
    // Simulated fetch for admin users
    setTimeout(() => {
      setUsers([
        { id: 1, name: 'Youssef Admin', email: 'admin@b2b.com', role: 'admin', joined: '10 Jan 2026', status: 'Active' },
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-30 pt-16">
        <div className="max-w-7xl mx-auto px-8 flex gap-8">
          <button onClick={() => setCurrentPage('admin-dashboard')} className={`py-4 text-sm font-bold border-b-2 transition-colors ${currentPage === 'admin-dashboard' ? 'border-orange-600 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Dashboard</button>
          <button onClick={() => setCurrentPage('admin-products')} className={`py-4 text-sm font-bold border-b-2 transition-colors ${currentPage === 'admin-products' ? 'border-orange-600 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Products</button>
          <button onClick={() => setCurrentPage('admin-orders')} className={`py-4 text-sm font-bold border-b-2 transition-colors ${currentPage === 'admin-orders' ? 'border-orange-600 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Orders</button>
          <button onClick={() => setCurrentPage('admin-users')} className={`py-4 text-sm font-bold border-b-2 transition-colors ${currentPage === 'admin-users' ? 'border-orange-600 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Users</button>
        </div>
      </div>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button 
                            ...
                            className={`p-2 rounded-lg transition-all ${user.status === 'Active' ? 'text-gray-400 hover:text-red-600 hover:bg-red-50' : 'text-green-600 bg-green-50 hover:bg-green-100'}`}
